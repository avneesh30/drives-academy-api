...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const [newCategory] = await db("rules_categories")
        .insert({ title })
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const categories = await db("rules_categories").select("*");
      res.json(categories);
    } catch (error) {
      console.error(error);
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const category = await db("rules_categories").where({ id }).first();
      if (!category) {
...
router.put(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const { title } = req.body;
      const existingCategory = await db("rules_categories")
        .where({ id })
        .first();
      if (!existingCategory) {
        res.status(404).json({ message: "Rules category not found" });
      ...
      const [updatedCategory] = await db("rules_categories")
        .where({ id })
        .update({ title })
        .returning(["id", "title", "created_at", "updated_at"]);
...
router.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      await db("rules_categories").where({ id }).del();
...
router.post("/content", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const [newContent] = await db("rules_content")
...
router.get("/content", async (req: Request, res: Response): Promise<void> => {
  try {
    const content = await db("rules_content").select("*");
    res.json(content);
  } catch (error) {
    console.error(error);
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const content = await db("rules_content").where({ id }).first();
      if (!content) {
...
router.put(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const { category_id, title, content } = req.body;
      const existingContent = await db("rules_content").where({ id }).first();
      if (!existingContent) {
        res.status(404).json({ message: "Rules content not found" });
        return;
      }
      const [updatedContent] = await db("rules_content")
        .where({ id })
...
router.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      await db("rules_content").where({ id }).del();
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const content = await db("rules_content")
      ...
      console.error(error);
      res.status(500).json({
        message: "Error fetching rules content for category",
      });
    }
  },
);
